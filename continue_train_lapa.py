...
if __name__ == "__main__":
    with open("config_v01c.json", "r") as f:
      config = json.load(f)

    # Dynamically pass the JSON keys to GPTHifiganConfig
    config = GPTHifiganConfig(**config)
    # config = GPTHifiganConfig(
    #     batch_size=32,  # Substantially increased for faster training and better gradient stability
    #     eval_batch_size=16,  # Increased evaluation batch size
    #     num_loader_workers=8,  # Higher number of workers to keep up with the larger batch sizes
    #     num_eval_loader_workers=4,  # Proportionally increased for efficient evaluation
    ...
    #     seq_len=32768,  # Keep sequence length constant
    ...
    #     use_noise_augment=True,  # Enable noise augmentation for better generalization
    #     eval_split_size=150,
    #     print_step=100,  # Log progress every 50 steps
    ...
    #     lr_gen=7.5e-5,  # Slightly reduced generator learning rate
    #     lr_disc=3.75e-5,  # Adjusteprint_step d discriminator learning rate proportionally
    ...
    #     # Pretrained model
    #     pretrain_path="/content/drive/MyDrive/AI/voice/Train_Hifigan_XTTS/outputs/run-December-16-2024_04+21PM-5acf424/best_model.pth",  # Updated path
