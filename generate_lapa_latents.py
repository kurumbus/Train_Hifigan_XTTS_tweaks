...
class GPTDecoder:
    def __init__(self, config, config_dataset):
        ...
        self.train_samples, _ = load_tts_samples(
            config_dataset,
            eval_split_size=0.02127659574468085,  # Use a very small percentage for evaluation
        )
...
if __name__ == "__main__":
    ...
    model_args = GPTArgs(
        max_conditioning_length=441000,  # 20 seconds
        min_conditioning_length=44100,   # 2 seconds
    ...
    dataset_ru = BaseDatasetConfig(
        formatter="ljspeech",
        dataset_name="lapa",
        path="/ai-voice/dataset_2",
        meta_file_train="/ai-voice/dataset_2/merged_metadata_cyr.csv",
        language="ru",
    )
    dataset_config = [dataset_ru]

    gpt_decode = GPTDecoder(config, dataset_config)
    gpt_decode.generate(output_dir="lapa_latents")
